...
### Open follow-ups discovered during the smoke

These were observed in the live smoke and partially addressed in subsequent micro-cycles. Track and address remaining items before declaring actual Stage-2 run results valid.

- [x] **Model quality under forced tool use.** Resolved by switching to `Qwen/Qwen3.6-35B-A3B-FP8` (April 2026 release; MoE 35B/3B-active; native `qwen3_coder` tool-call parser in vLLM). Phase 1 produces correct implementations: `8/8 hidden_pass` on slugify in `qwen36-smoke-2`. Phase 2 produces real (non-stub, non-ceiling) edits: `10/13 hidden_pass` on slugify post-edit, with 8 inherited behaviors preserved + 2 of 5 new `maxLength` behaviors passing — the kind of partial-credit signal the bench needs to distinguish methodologies.
- [x] **scoreHidden unit-of-measurement on slugify.** Resolved: `total_count` correctly reports the per-`test(...)` count (8 for slugify Phase 1, 13 for Phase 2 against `hidden_tests_after_edit/`). The earlier `total_count: 1` observation was the file-failed-to-load pattern — the test file's top-level `await import()` failed on a CJS-style source, counted as one file-level subtest failure. The ESM-instruction directive (commit `3b6fd2e`) eliminated that failure mode.
- [ ] **vLLM stability under multi-trial load.** vLLM's `EngineCore` periodically dies with `EngineDeadError` in IPC `dequeue` (commit `vllm-qwen36-5.log:04:50:56`) — a known multiprocess-executor issue, not OOM (KV cache was 3.2% at crash time). Mitigation in place: `bench/.local-serve/vllm-supervisor.sh` auto-restarts vLLM on crash; the bench's existing per-trial try/catch records the in-flight trial in `errors.jsonl` and the orchestrator continues to the next cell. Trade: ~5–10% of trials may land during the restart window; with N=10 trials per cell the missing-data rate is tolerable for v2 analysis. Investigate alternative servers (SGLang, TGI) if the per-run loss climbs above ~15%.
- [ ] **Phase 2-WD failure mode** (when WD path overlapped with the vLLM crash in `qwen36-smoke-2`). With the supervisor in place, re-run a smoke and verify Phase 2-WD completes. If it consistently fails when Phase 2 succeeds with the same model, surface as a separate issue (could be intent.md+edit.md token-shape triggering vLLM crash).
