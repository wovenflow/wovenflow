// Hidden tests for the deep-merge task — POST-EDIT version.
// All tests from hidden_tests/ are included verbatim (regression check),
// plus new assertions for the additive edit (options.arrayMerge per edit.md).
...
// --- post-edit assertions: options.arrayMerge === 'concat' ---

test('[concat-arrays] both-array values are concatenated under concat mode', () => {
  const r = deepMerge({ items: [1, 2] }, { items: [3, 4] }, { arrayMerge: 'concat' });
  assert.deepEqual(r, { items: [1, 2, 3, 4] });
});

test('[concat-default-replace] without concat option arrays still replace', () => {
  const replaced = deepMerge({ items: [1, 2] }, { items: [3, 4] });
  assert.deepEqual(replaced, { items: [3, 4] });
  // an unknown arrayMerge value also falls back to replace
  const replacedToo = deepMerge({ items: [1, 2] }, { items: [3, 4] }, { arrayMerge: 'replace' });
  assert.deepEqual(replacedToo, { items: [3, 4] });
});

test('[concat-only-both-arrays] concat applies only when both sides are arrays', () => {
  // array in target, object in source -> source still wins (no concat)
  const a = deepMerge({ x: [1, 2] }, { x: { y: 1 } }, { arrayMerge: 'concat' });
  assert.deepEqual(a, { x: { y: 1 } });
  // object in target, array in source -> source still wins (no concat)
  const b = deepMerge({ x: { y: 1 } }, { x: [3, 4] }, { arrayMerge: 'concat' });
  assert.deepEqual(b, { x: [3, 4] });
});

test('[concat-no-mutation] concatenation does not mutate either input array', () => {
  const target = { items: [1, 2] };
  const source = { items: [3, 4] };
  const r = deepMerge(target, source, { arrayMerge: 'concat' });
  assert.deepEqual(r.items, [1, 2, 3, 4]);
  assert.deepEqual(target.items, [1, 2]);
  assert.deepEqual(source.items, [3, 4]);
});

test('[concat-nested] concat applies recursively to nested arrays', () => {
  const r = deepMerge(
    { a: { tags: ['x'] } },
    { a: { tags: ['y', 'z'] } },
    { arrayMerge: 'concat' },
  );
  assert.deepEqual(r, { a: { tags: ['x', 'y', 'z'] } });
});
