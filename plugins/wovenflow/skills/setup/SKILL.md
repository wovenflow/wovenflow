...
- [**`wovenflow:setup` claims template**](https://github.com/wovenflow/wovenflow) `(Recommended for research projects)` — materializes `claims.md.tmpl` into `<repo>/{{CLAIMS_FILE}}` (default `claims.md`) and creates `{{CLAIMS_DIR}}` (default `docs/claims/`) with `claims-doc.md.tmpl` as the per-claim template. A capped table of what the project might be able to **assert**, scoring **novelty and evidence in separate columns** — a claim can be wide open in the literature and wholly unevidenced, and collapsing the two into one "confidence" number is how a project talks itself into a claim it cannot defend. Written whenever a search or a measurement moves a row; read at every `startup`. Skip it for projects making no external claim — leaving `{{CLAIMS_FILE}}` empty strips it out of `startup`.
...
| `prior-work.md.tmpl` | `<repo>/PRIOR-WORK.md` | Project does any literature or paper searching. Capped one-line index of external prior work, written by `researchflow` step 10 and read at every startup; guards against writing a claim from recollection when a source exists. |
| `claims.md.tmpl` | `<repo>/{{CLAIMS_FILE}}` | Project intends to claim something externally — a paper, a launch, a standards proposal. **Capped** table of candidate claims with novelty and evidence as SEPARATE columns; an evidence cell cites a commit, a novelty cell cites what was searched. Pairs with `prior-work.md.tmpl`: that one indexes what others did, this one what we could say. |
| `tasks-doc.md.tmpl` | `<repo>/{{TASKS_DIR}}/<id>-<slug>.md` | The long form behind one task row — what it is and why, what constrains it, the working log appended with `tasks note`, and links to the specs it produced. Created for **every** task, not only the ones that outgrow a cell: it is where work gets written down, so it has to exist before there is anything to write. |
| `claims-doc.md.tmpl` | `<repo>/{{CLAIMS_DIR}}/<id>-<slug>.md` | The long explanation behind one claim row, created when a row outgrows its cell. Sections for what it asserts, what the literature says, what we measured, what would move it, and **what would sink it**. |
...
5a. **Also create `<repo>/{{TASKS_DIR}}`** (default `docs/tasks/`) and materialize `tasks-doc.md.tmpl` into it as `T-001-example.md`, matching the pointer in the starter row. **The task index is an index like the other three**: the row carries status, owner, priority and a one-line hook, and everything written *about* the task — the description, what blocks it, the design, what was ruled out and why — lives in `{{TASKS_DIR}}/<id>-<slug>.md`.

   Say why, because "another directory" reads as ceremony. The failure it prevents is specific and it is what a task tracker reliably does: the Notes cell is the only writable place in the row, so every design note, amendment and quoted decision ends up crammed into it. In the project where this shape was worked out, one cell reached **3,134 characters on a single line** and the file was 39 KB of table; split, the index was 15 KB. `{{RECORDS_HELPER}} lint tasks` fails a row whose hook has grown past 240 characters, and the message names the doc to move it into — **nothing is deleted, it moves**, which is what separates this from the optional line caps.

   Two rules differ from the claim docs, both deliberate. **Every task gets a file, including the short ones** — a stub in the claims directory is a claim to have thought about something, but a task is where work happens and needs somewhere to write *before* there is anything to write, so `tasks add` creates the row and the doc together. And the task doc reads **oldest first**: it is a working record, where the findings index is a lookup and the newest entry is the one you came for.
...
7b. **Special case for projects that intend to CLAIM something** — a paper, a launch announcement, a standards proposal, a grant report. Materialize `claims.md.tmpl` → `<repo>/{{CLAIMS_FILE}}` (default `claims.md`) and create `<repo>/{{CLAIMS_DIR}}` (default alongside `{{PRIOR_WORK_DIR}}`, e.g. `docs/claims`), copying `claims-doc.md.tmpl` in as the per-claim template. **Never overwrite.**
   ...
   Ask for `{{CLAIMS_FILE}}`, `{{CLAIMS_DIR}}` and `{{CLAIMS_CAP}}`. **Leave `{{CLAIMS_FILE}}` empty to disable it** — same mechanism as prior work, and the right answer for internal tooling with nothing to assert.

   Say what it is for, because it looks like a roadmap and is not. It scores **novelty and evidence in separate columns**, and the separation is the whole mechanism: a claim can be wide open in the literature and completely unevidenced by us, which *feels* like a contribution and is the weakest square on the board. An evidence cell cites a **commit**; a novelty cell cites **what was searched**. Two further rules that are learned the hard way: `?` unsearched is a **liability**, not a blank claim of novelty — searching can only lower it; and a headline claim that is a **conjunction inherits the minimum of its links**, never the maximum.
